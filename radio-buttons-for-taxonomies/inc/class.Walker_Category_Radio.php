...
class Walker_Category_Radio extends Walker {
	...
	public function start_el( &$output, $category, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( ! empty( $args['list_only'] ) ) {
			...
			/** This filter is documented in wp-includes/category-template.php */
			$children = get_term_children($category->term_id,'rugcat-tax');
			if(empty($children)){
				$output .= "\n<li id='{$taxonomy}-{$category->term_id}'$class>" ;
				$output .=	'<label class="selectit">';
				$output .='<input value="' . intval( $category->term_id ) . '" type="radio" name="'.esc_attr( $name ).'[]" id="in-'.esc_attr( $taxonomy ).'-' . intval( $category->term_id ) . '"' .
				...
				$output .= "\n<li id='{$taxonomy}-{$category->term_id}'$class>" ;
				$output .=	'';
				$output .='<input style = "display:none;" value="' . intval( $category->term_id ) . '" type="radio" name="'.esc_attr( $name ).'[]" id="in-'.esc_attr( $taxonomy ).'-' . intval( $category->term_id ) . '"' .
						checked( $category->term_id, $selected_id, false ) .
						disabled( empty( $args['disabled'] ), false, false ) . ' /> ';

				$output .= '<label class="selectit">'.esc_html( apply_filters( 'the_category', $category->name ) ) . '</label>';
